...
export function storeTupleArray(source: Array) {
  ...
  builder.writeCell(
    source.m.size > 0
      ? beginCell()
          .storeDictDirect(source.m, Dictionary.Keys.Uint(16), Dictionary.Values.BigInt(257))
          .endCell()
      : null
  )
...
export function loadCreateUserRequest(slice: Slice) {
  const sc_0 = slice
  if (sc_0.loadUint(32) !== 3663894300) {
    throw Error('Invalid prefix')
  }
  const _wallet = sc_0.loadAddress()
  const _publicKey = sc_0.loadUintBig(256)
  return { $$type: 'CreateUserRequest' as const, wallet: _wallet, publicKey: _publicKey }
}
...
export function loadUserFactory$Data(slice: Slice) {
  const sc_0 = slice
  return { $$type: 'UserFactory$Data' as const }
}
