...
type CertificateNFT_init_args = {
  $$type: 'CertificateNFT_init_args'
  collection: Address
  index: bigint
}

function initCertificateNFT_init_args(src: CertificateNFT_init_args) {
  return (builder: Builder) => {
    const b_0 = builder
    b_0.storeAddress(src.collection)
    b_0.storeInt(src.index, 257)
...
async function CertificateNFT_init(collection: Address, index: bigint) {
  ...
  initCertificateNFT_init_args({ $$type: 'CertificateNFT_init_args', collection, index })(builder)
  const __data = builder.endCell()
  return { code: __code, data: __data }
}

export const CertificateNFT_errors = {
...
export const CertificateNFT_errors_backward = {
  'Stack underflow': 2,
  'Stack overflow': 3,
  'Integer overflow': 4,
...
const CertificateNFT_types: ABIType[] = [
  {
    name: 'DataSize',
    header: null,
    fields: [
...
const CertificateNFT_receivers: ABIReceiver[] = [
...
export class CertificateNFT implements Contract {
  public static readonly storageReserve = 0n
  public static readonly errors = CertificateNFT_errors_backward
  public static readonly opcodes = CertificateNFT_opcodes

  static async init(collection: Address, index: bigint) {
    return await CertificateNFT_init(collection, index)
  }

  static async fromInit(collection: Address, index: bigint) {
    const __gen_init = await CertificateNFT_init(collection, index)
    const address = contractAddress(0, __gen_init)
    return new CertificateNFT(address, __gen_init)
  }

  static fromAddress(address: Address) {
    return new CertificateNFT(address)
  }

  readonly address: Address
  readonly init?: { code: Cell; data: Cell }
  readonly abi: ContractABI = {
    types: CertificateNFT_types,
    getters: CertificateNFT_getters,
    receivers: CertificateNFT_receivers,
    errors: CertificateNFT_errors,
  }
